        ...
        : Node("icp_node")
    {
        ...
        if(pcl_type == "livox")
        {
            lvx_cloud_sub_ = this->create_subscription<livox_ros_driver2::msg::CustomMsg>(
                "/livox/lidar1", 10, std::bind(&ICPNode::lvx_cloud_callback, this, std::placeholders::_1));
        }
        else
        {
...
private:
    void cloud_callback(const sensor_msgs::msg::PointCloud2::SharedPtr msg)
    {
        ...
        extrinsic(0, 3) = -0.05; // Translation along X-axis
        extrinsic(1, 3) = 0.0;    // Translation along Y-axis
        extrinsic(2, 3) = 1.0345;    // Translation along Z-axis

        // Set the rotation components (Identity matrix in this case)
        extrinsic(0, 0) = 1.0; extrinsic(0, 1) = 0.0; extrinsic(0, 2) = 0.0;
        extrinsic(1, 0) = 0.0; extrinsic(1, 1) = 1.0; extrinsic(1, 2) = 0.0;
        extrinsic(2, 0) = 0.0; extrinsic(2, 1) = 0.0; extrinsic(2, 2) = 1.0;

        // Apply the transformation to the input cloud
        pcl::transformPointCloud(*input_cloud, *input_cloud, extrinsic);
