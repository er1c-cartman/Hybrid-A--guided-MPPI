...
bool SmacPlannerHybrid::IsValid_Occ(int start_mx, int start_my, int goal_mx, int goal_my)
{
  int offset_x = goal_mx - start_mx;
  int offset_y = goal_my - start_my;
  int occ_center = 49;

  if(occ_center + offset_x > 99 || occ_center + offset_y < 0 || occ_center + offset_y > 99 || occ_center + offset_y < 0)
  {
    return false;
  }
  else
...
void SmacPlannerHybrid::costMapCB(const nav_msgs::msg::OccupancyGrid::SharedPtr msg)
{
  int it = 0;
  int rows = 0;
  ...
  double resolution = static_cast<double>(msg->info.resolution);
  geometry_msgs::msg::Pose origin;

  origin = msg->info.origin;

  // std::cout << "Traversability Map:\n" << traversability_map << std::endl;

  
  costMap_.resize(width,height);
  for(int i=0;i<msg->data.size();i++)
  {
      costMap_(width-1-rows, it) = static_cast<double>(msg->data[i]);
      it++;
      if(it==100)
      {
          it=0;
  ...
  //   {
  //     int idx = y * width + x;
  //     costMap_(y, x) = static_cast<double>(msg->data[idx]);
  //   }
  // }
  ...
  std::shared_ptr<nav2_costmap_2d::Costmap2DROS> costmap_ros)
{
  ...
  //
  subscription_occMap_ = node->create_subscription<nav_msgs::msg::OccupancyGrid>(
    "traversability_occ_map", 10, std::bind(&SmacPlannerHybrid::costMapCB, this, std::placeholders::_1));
  ...
  // OCC MAP Subs
  // auto subscription_occMap = node->create_subscription<nav_msgs::msg::OccupancyGrid>("traversability_occ_map", 10, std::bind(&SmacPlannerHybrid::costMapCB, this, std::placeholders::_1));
...
void SmacPlannerHybrid::activate()
{
  ...
  // Add callback for dynamic parameters
    // Move the subscription here to ensure it's active during the planning
  ...
  const geometry_msgs::msg::PoseStamped & goal)
{
  ...
  // Start Position 을 Map 의 Grid Index 로 변경해줌 --> mx, my
  ...
  */
  ...
  // Goal Position 을 Map 의 Grid Index 로 변경해줌 --> mx, my
  ...
  // std::string filename = "/home/chan/Astar_ws/src/costMap.txt";
  // this->saveMatrixToFile(filename,costMap_);
  ...
  // 내 생각엔 여기서 Goal Index 를 한번 변경해줘야함
  // Get Local Goal 함수를 이용
  // 만약 Start mx 를 새로운 Local Map 으로 놓고
  // 99 + offset > 199 or 99+offset < 0 이면 Local Goal 설정
  // 계속 업데이트 되고있음, + 잘 되는 듯?
  bool res;
  res = IsValid_Occ(start_mx, start_my, goal_mx, goal_my);
  if(res)
  {
    // std::cout<<"Goal In the OCC Map"<<std::endl;
  }
  else 
  {
    // std::cout<<"Need To Local Goal "<<std::endl;
  }

  // std::cout<<"Start pose x : "<<start.pose.position.x<<std::endl;
  // std::cout<<"Start pose y : "<<start.pose.position.y<<std::endl;
  // std::cout<<"Start Mx : "<<start_mx<<std::endl;
  // std::cout<<"Start My : "<<start_my<<std::endl;
  // std::cout<<"Goal pose x : "<<goal.pose.position.x<<std::endl;
  // std::cout<<"Goal pose y : "<<goal.pose.position.y<<std::endl;
  // std::cout<<"Goal Mx : "<<goal_mx<<std::endl;
  // std::cout<<"Goal My : "<<goal_my<<std::endl;



  // _a_star->getOccMap(costMap_);
  ...
  try {
    if (!_a_star->createPath(
        path, num_iterations, _tolerance / static_cast<float>(costmap->getResolution()),costMap_))
    {
...
void SmacPlannerHybrid::saveMatrixToFile(const std::string& filename,const Eigen::MatrixXd& matrix)
{
