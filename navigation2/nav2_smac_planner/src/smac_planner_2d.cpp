...
bool SmacPlanner2D::IsValid_Occ(int start_mx, int start_my, int goal_mx, int goal_my)
{
  int offset_x = goal_mx - start_mx;
  int offset_y = goal_my - start_my;
  int occ_center = 99;

  if(occ_center + offset_x>200 || occ_center + offset_y<0 || occ_center+offset_y > 200 || occ_center + offset_y <0)
  {
    return false;
  }
  else
...
void SmacPlanner2D::costMapCB(const nav_msgs::msg::OccupancyGrid::SharedPtr msg)
{
  int it = 0;
  int rows = 0;
  ...
  std::shared_ptr<nav2_costmap_2d::Costmap2DROS> costmap_ros)
{
  ...
  subscription_occMap_ = node->create_subscription<nav_msgs::msg::OccupancyGrid>(
    "traversability_occ_map", 10, std::bind(&SmacPlanner2D::costMapCB, this, std::placeholders::_1));
  ...
  const geometry_msgs::msg::PoseStamped & goal)
{
  ...
  bool res;
  res = IsValid_Occ(mx_start, my_start, mx_goal, my_goal);
  if(res)
  {
