...
double Node2D::getOccCost(Coordinates node_coord, Eigen::MatrixXd occMap, unsigned int start_mx, unsigned start_my)
{
  int offset_x = static_cast<int>(node_coord.x) - start_mx;
  int offset_y = static_cast<int>(node_coord.y) - start_my;

  double occCost = occMap(99-offset_y, 99 + offset_x);

  if(99-offset_y>199 || 99 - offset_y <0 || 99+offset_x>199 || 99+offset_x <0)
  {
    return 0;
  } 
  else if(occCost<0) return 10;
  else if(occCost>60) return 1e+5;
  else return occCost;
}
