...
double NodeHybrid::getOccCost(Coordinates node_coord, Eigen::MatrixXd occMap, unsigned int start_mx, unsigned start_my)
{
  int offset_x = static_cast<int>(node_coord.x) - start_mx;
  int offset_y = static_cast<int>(node_coord.y) - start_my;

  double occCost = occMap(49 - offset_y, 49 + offset_x);

  if(49 - offset_y > 99 || 49 - offset_y < 0 || 49 + offset_x > 99 || 49 + offset_x  < 0) return 30;
  else if(occCost<0) return 30;
  else if(occCost>50) return 1e+5;
  else return occCost;
}
  ...
  NodeVector & neighbors)
{
  ...
  for (unsigned int i = 0; i != motion_projections.size(); i++) {
    ...
    // std::cout<<"motion_projections[i]._x"<<motion_projections[i]._x<<std::endl;
    // std::cout<<"motion_projections[i]._y"<<motion_projections[i]._y<<std::endl;
    int offset_x = motion_projections[i]._x - start_mx;
    // std::cout << "motion_map_start" << start_mx << std::endl;
    int offset_y = motion_projections[i]._y - start_my;
    
    // bool occ_valid;
    
    // if(99-offset_y>199 || 99-offset_y <0 || 99+offset_x>199 || 99+offset_x <0)
    // {
    //   std::cout<<"So Far need to local goal"<<std::endl;
    //   occ_valid = true;
    // }

    // else if(occMap(99-offset_y, 99+offset_x)>40 || occMap(99-offset_y, 99+offset_x)<0)
    // {
    //   occ_valid = false;
    //   // std::cout<<"NO OCC VALID !!"<<occMap(99-offset_y, 99+offset_x)<<std::endl;
    // }
    // else
    //   occ_valid = true;
