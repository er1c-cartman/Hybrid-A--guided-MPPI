...
template<typename NodeT>
void AStarAlgorithm<NodeT>::getOccMap(Eigen::MatrixXd occMap)
{
  occMap_.resize(occMap.rows(), occMap.cols());
  occMap_ = occMap;
} // Get Occ
  ...
  const unsigned int & dim_3)
{
  ...
  // Debug
  // Coordinates new_coord = _start ->getPose();
  
  // std::cout<<"Coord X : "<<new_coord.x<<std::endl;
  // std::cout<<"Coord X : "<<new_coord.y<<std::endl;
  // std::cout<<"Coord X : "<<new_coord.theta<<std::endl;
...
bool AStarAlgorithm<NodeT>::createPath(
  ...
  const float & tolerance, Eigen::MatrixXd costMap)
{
  ...
  //Test
  // std::cout<<"Index X : "<<_start->x<<std::endl;
  // std::cout<<"Index Y : "<<_start->y<<std::endl;
  // std::cout<<"Index Yaw : "<<_start->dim_3<<std::endl;
  ...
  // std::cout<<"Get Index "<<_start->getIndex()->x<<std::endl;
  ...
  while (iterations < getMaxIterations() && !_queue.empty()) {
    ...
    unsigned int tmp_idx = current_node->getIndex();
    Coordinates tmp_coord = current_node->getCoords(tmp_idx, getSizeX(), getSizeDim3());
    // std::cout << "Current Node X: " << tmp_coord.x << std::endl;
    // std::cout << "Current Node Y: " << tmp_coord.y << std::endl;
    ...
    // NodeHybrid::Coordinates new_coord = _start->getCoords(_start->getIndex());
    // std::cout << "Start Node Index: " << _start->pose.x << std::endl;
    ...
    neighbors.clear();
    current_node->getNeighbors(neighborGetter, _collision_checker, _traverse_unknown, costMap, start_mx_, start_my_,neighbors); //get neighbor 를 2d로 할건가 hybrid 로 할건가 아니면 lattice로 할건가 // 우리는 2d로 할거임
      ...
      neighbor_iterator != neighbors.end(); ++neighbor_iterator)
    {
      ...
      Coordinates neighbor_coord = neighbor->getCoords(neighbor->getIndex(), getSizeX(), getSizeDim3());
      // test_cost = current_node->getOccCost(neighbor_coord, costMap, start_mx_, start_my_);
      g_cost = (current_node->getAccumulatedCost() + current_node->getTraversalCost(neighbor) + 0.5 * current_node->getOccCost(neighbor_coord, costMap, start_mx_, start_my_)); //local에서는 0.5임//getOccCost 도 2d 에다가 박아야함 // smac_planner_hybrid.cpp:  // _a_star->getOccMap(costMap_); 에서 호출됨 그럼 나는 smac_planner_2d.cpp에 박아야함
